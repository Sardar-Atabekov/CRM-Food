...
class Departments extends Component {
  ...
  async componentDidMount() {
    getData("https://neobiscrmfood.herokuapp.com/api/Waiter/getMenu").then(
      body => {
  ...
  render() {
    let { data } = this.state,
      barDepartment = data.filter(
        department => department.departmentName === "Бар"
      ),
      cookDepartment = data.filter(
        department => department.departmentName === "Кухня"
      );
